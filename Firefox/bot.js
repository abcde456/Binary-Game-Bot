console.log("Binary Bot Initialized");

let botOn = false;

chrome.storage.local.get("botOn", (data) => {
    if (data.hasOwnProperty("botOn")) {
        botOn = data.botOn;
        console.log("Bot restored state from storage:", botOn);
    }
});

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "TOGGLE_BOT") {
        botOn = message.enabled;
        console.log("Bot received toggle:", botOn);
    }
});
...
const watcher = setInterval(() => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (btn) btn.addEventListener("click", switchBotState);
    });
}, 10);
